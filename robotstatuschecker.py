        ...
        if expected.msg_index != "*":
            ...
            for msg in kw.messages:
                if self._check_msg_message(test, kw, msg, expected, fail=False):
                    self._check_msg_level(test, kw, msg, expected)
                    break
            else:
                message = f"Keyword '{kw.name}' (index {expected.kw_index_str}) does not contain any logs with message '{expected.message}'"
                self._fail(test, message)
